...
def norm(t):
    t_clean = html.unescape(t).replace('’', "'").replace('', "'")
...
for t in tags_text:
    ...
    if clean_t and len(clean_t) > 3 and not clean_t.startswith('import') and not clean_t.startswith('return') and not clean_t.startswith('const') and re.search(r'[a-zA-Z0-9]', clean_t):
        strings_to_check.append((clean_t, 'JSX Text Node'))

# Extract option text nodes in selects
option_texts = re.findall(r'<option[^>]*>([^<]+)</option>', page_clean)
for opt in option_texts:
    clean_opt = html.unescape(opt.strip())
    if clean_opt and len(clean_opt) > 3 and 'Select' not in clean_opt:
        strings_to_check.append((clean_opt, 'Option Text'))
...
print("="*70)
print(f"CONTACT PAGE STRICT DOCX AUDIT RESULT:")
