...
# Remove JSX comments & JavaScript code blocks inside { ... }
home_code_clean = re.sub(r'\{/\*.*?\*/\}', '', home_code, flags=re.DOTALL)

strings_to_check = []

# Extract strings in JSX arrays
arrays = re.findall(r'\[(.*?)\]', home_code_clean, re.DOTALL)
for arr in arrays:
...
# Extract pure text nodes between > and <, skipping code blocks containing { or }
tags_text = re.findall(r'>([^<{}>]+)<', home_code_clean, re.DOTALL)
for t in tags_text:
...
print("="*70)
print(f"HOMEPAGE STRICT DOCX AUDIT RESULT:")
